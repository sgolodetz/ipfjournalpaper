...
%To see why, let $P(n_1,...,n_k)$ be the probability that merging nodes $n_1,...,n_k$ will involve at least two levels of unzipping. Then (from the data) $P(n_i,n_j) \approx \frac{1}{2}$, and based on an assumption of independence:
%
%\[
%P(n_1,...,n_k) = 1 - \frac{1}{2}^{{}^kC_2}
%\]
%
%To see why, take the probability that the merge of two arbitrary adjacent nodes $n_i$ and $n_j$ will involve at least two levels of unzip to be around $\frac{1}{2}$, and make the reasonable (if not wholly true) assumption of independence between pairwise node merges. If $P(n_1,...,n_k)$ is the probability 

%Restricting the user to sibling-only node merging would thus prohibit the majority of the potential merges they might want to perform. Bearing in mind the relatively high input requirements for splitting nodes, and the consequent costs of performing a non-sibling node merge manually as a sequence of node splits and sibling node merges, there is hence a need for a less restrictive merge algorithm of the kind described in this paper.

%\item For each IPF, there was also a relatively high cumulative percentage ($\approx 50\%$) of node pairs that required at least two levels of unzipping to merge. That is, around half of all the pairwise node merges the user might want to perform on the analysed IPFs would be particularly difficult and time-consuming to effect without non-sibling node merging. With non-sibling node merging, each pairwise node merge would be a simple operation in the UI (select the two nodes and click to merge), with the potential to 
\end{itemize}
%
