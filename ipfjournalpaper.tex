...
\begin{abstract}
The segmentation of medical scans and the subsequent identification of
key features therein is an important precursor to many medical imaging
applications. One interesting approach is to start by partitioning the
image into a region hierarchy, in which each node represents a
contiguous region of the image; however, once built, the hierarchy
tends to be static, making the results very dependent on the initial
tree construction process. In this paper, we describe our approach to
the automatic feature identification problem, in particular explaining
why modifying the hierarchy at a later stage can be useful, and how it
can be achieved.
\end{abstract}
